<template>
  <div class="credential-fields">
    <div class="credential-fields__title">{{ title }}</div>
    <t-alert
      v-if="!fields.length"
      theme="info"
      :message="emptyHint"
      class="credential-fields__alert"
    />
...
// 三端共用的动态凭证表单（支付渠道 / 通知渠道 / 验证码服务商）。
//
// 逻辑：读描述符的 credential_schema → 渲染 input/password/select/switch/number/textarea
// → 密文字段回显脱敏 → 未改则原样回传（后端 IsMaskedEcho 识别为「未修改」）。
// 组件不关心业务域，只认字段声明，因此可被三个模块直接复用。
