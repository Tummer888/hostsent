<template>
  ...
  >
    ...
    <div class="settings-body">
      ...
      <!-- 布局 -->
      <section class="settings-section">
        <h4 class="settings-section__title">布局</h4>
        <div class="settings-row">
          <span>显示标签栏</span>
          <t-switch
            :value="settings.showTabsBar"
            @change="(v: unknown) => settings.update({ showTabsBar: Boolean(v) })"
          />
        </div>
        <div class="settings-row">
          <span>记忆标签页（刷新后保留）</span>
          <t-switch
            :value="settings.rememberTabs"
            @change="(v: unknown) => settings.update({ rememberTabs: Boolean(v) })"
          />
        </div>
      </section>
      ...
      <!-- 导航菜单 -->
      <section class="settings-section">
        <h4 class="settings-section__title">导航菜单</h4>
        <div class="settings-row">
          <span>菜单手风琴（同时只展开一项）</span>
          <t-switch
            :value="settings.menuAccordion"
            @change="(v: unknown) => settings.update({ menuAccordion: Boolean(v) })"
          />
        </div>
        <div class="settings-row settings-row--stack">
          <span>点击一级菜单</span>
          <t-radio-group
            :value="settings.firstMenuClick"
            variant="default-filled"
            @change="(v: unknown) => settings.update({ firstMenuClick: v as FirstMenuClickMode })"
          >
            <t-radio-button value="switch">仅切换</t-radio-button>
            <t-radio-button value="navigate">跳转页面</t-radio-button>
          </t-radio-group>
        </div>
      </section>
    </div>
...
import type { ColorScheme, FirstMenuClickMode } from '@/store/modules/settings'
import { DEFAULT_THEME_COLOR, PRESET_THEME_COLORS, normalizeHex } from '@/utils/theme'
