<template>
  <div class="page-body ticket-module">
    <header class="page-header surface-card">
      <div class="page-header__main">
        <span class="page-header__chip">
          <ChartIcon size="22" aria-hidden="true" />
        </span>
        <div class="page-header__text">
          <h2 class="page-header__title">工单统计</h2>
        </div>
      </div>
      <t-space size="small">
        <t-button variant="outline" :loading="loading" @click="loadStats">
          <template #icon>
            <RefreshIcon aria-hidden="true" />
          </template>
          刷新
        </t-button>
      </t-space>
    </header>

    <section class="stat-grid">
      <div
        v-for="stat in metricCards"
        :key="stat.key"
        class="stat-card surface-card"
        :class="`stat-card--${stat.variant}`"
      >
        <span class="stat-card__icon">
          <component :is="stat.icon" size="22" aria-hidden="true" />
        </span>
        <div class="stat-card__info">
          <span class="stat-card__value">{{ stat.value }}</span>
          <span class="stat-card__label">{{ stat.label }}</span>
        </div>
      </div>
    </section>

    <section class="chart-grid">
      <div class="surface-card chart-card">
        <div class="table-card__head">
          <h3 class="card-title">近 7 日工单量趋势</h3>
          <span class="table-card__meta">每日新增工单</span>
        </div>
        <EChart :option="trendOption" height="300" />
      </div>

      <div class="surface-card chart-card">
        <div class="table-card__head">
          <h3 class="card-title">分类分布</h3>
          <span class="table-card__meta">工单分类占比</span>
        </div>
        <EChart v-if="hasCategoryData" :option="categoryPieOption" height="300" />
        <div v-else class="chart-empty">
          <t-empty description="暂无工单数据" />
        </div>
      </div>
    </section>

    <section class="surface-card chart-card">
      <div class="table-card__head">
        <h3 class="card-title">状态明细</h3>
        <span class="table-card__meta">按工单状态统计</span>
      </div>
      <div class="dist-list">
        <div v-for="dist in distributions" :key="dist.status" class="dist-row">
          <div class="dist-row__head">
            <span class="dist-row__name">{{ ticketStatusLabel(dist.status) }}</span>
            <span class="dist-row__value">{{ dist.count }}</span>
          </div>
          <div class="dist-row__track">
            <div class="dist-row__bar" :style="{ width: dist.percent + '%', background: dist.color }"></div>
          </div>
        </div>
        <t-empty v-if="distributions.length === 0" description="暂无工单数据" />
      </div>
    </section>
  </div>
</template>

<script setup lang="ts">
import { computed, markRaw, onMounted, ref } from 'vue'

import { ChartIcon, CheckCircleIcon, ErrorCircleIcon, RefreshIcon, SwapIcon, TimeIcon } from 'tdesign-icons-vue-next'
import { MessagePlugin } from 'tdesign-vue-next'
import type { EChartsOption } from 'echarts'

import EChart from '@/components/EChart.vue'
import { getTicketStats } from '@/api/ticket'
import { formatDuration, ticketStatusLabel } from '@/pages/ticket/constants'
import type { TicketStatsResponse } from '@/types/interface'

defineOptions({ name: 'TicketStats' })

const loading = ref(false)
const stats = ref<TicketStatsResponse | null>(null)

const STAT_COLORS = ['#f59e0b', '#2563eb', '#8b5cf6', '#16a34a', '#64748b', '#dc2626', '#0891b2', '#ec4899']

const metricCards = ref([
  { key: 'open', label: '待处理工单', value: '0', icon: markRaw(ErrorCircleIcon), variant: 'orange' },
  { key: 'in_progress', label: '处理中工单', value: '0', icon: markRaw(SwapIcon), variant: 'blue' },
  { key: 'resolved', label: '已解决工单', value: '0', icon: markRaw(CheckCircleIcon), variant: 'success' },
  { key: 'avg_first_reply', label: '平均首次响应', value: '—', icon: markRaw(TimeIcon), variant: 'purple' },
])

// 近 7 日趋势柱状图
const trendOption = computed<EChartsOption>(() => {
  const trend = stats.value?.trend ?? []
  const dates = trend.map((t) => t.date.slice(5))
  const counts = trend.map((t) => t.count)
  return {
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    grid: { left: 8, right: 8, top: 24, bottom: 8, containLabel: true },
    xAxis: {
      type: 'category',
      data: dates,
      axisLine: { lineStyle: { color: '#e2e8f0' } },
      axisTick: { show: false },
      axisLabel: { color: '#475569', fontSize: 12 },
    },
    yAxis: {
      type: 'value',
      minInterval: 1,
      axisLabel: { color: '#94a3b8' },
      splitLine: { lineStyle: { color: '#eef2f7' } },
    },
    series: [
      {
        name: '新增工单',
        type: 'bar',
        data: counts,
        barMaxWidth: 24,
        itemStyle: {
          borderRadius: [4, 4, 0, 0],
          color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: '#38bdf8' }, { offset: 1, color: '#2563eb' }] },
        },
      },
    ],
  }
})

// 分类分布饼图
const categoryPieData = computed(() => {
  const distribution = stats.value?.category_distribution ?? []
  return distribution.map((item) => ({ name: item.category, value: item.count }))
})

const hasCategoryData = computed(() => categoryPieData.value.some((item) => item.value > 0))

const categoryPieOption = computed<EChartsOption>(() => ({
  tooltip: { trigger: 'item', formatter: '{b}: {c} ({d}%)' },
  legend: { bottom: 0, icon: 'circle', textStyle: { color: '#64748b', fontSize: 12 } },
  series: [
    {
      name: '工单分类',
      type: 'pie',
      radius: ['46%', '70%'],
      center: ['50%', '44%'],
      avoidLabelOverlap: true,
      itemStyle: { borderRadius: 6, borderColor: '#fff', borderWidth: 2 },
      label: { show: true, formatter: '{b}\n{c}', fontSize: 12, color: '#334155' },
      emphasis: {
        label: { show: true, fontSize: 14, fontWeight: 'bold' },
        itemStyle: { shadowBlur: 10, shadowColor: 'rgba(0,0,0,0.12)' },
      },
      data: categoryPieData.value,
    },
  ],
}))

// 状态明细分布条
const distributions = computed(() => {
  const distribution = stats.value?.status_distribution ?? []
  const total = distribution.reduce((sum, item) => sum + item.count, 0)
  return distribution.map((item, index) => ({
    status: item.status,
    count: item.count,
    percent: total > 0 ? Math.round((item.count / total) * 100) : 0,
    color: STAT_COLORS[index % STAT_COLORS.length],
  }))
})

function loadMetricCards() {
  const data = stats.value
  if (!data) return
  metricCards.value[0].value = String(data.open)
  metricCards.value[1].value = String(data.in_progress)
  metricCards.value[2].value = String(data.resolved)
  metricCards.value[3].value = formatDuration(data.avg_first_reply_seconds)
}

async function loadStats() {
  loading.value = true
  try {
    stats.value = await getTicketStats()
    loadMetricCards()
  } catch (error) {
    MessagePlugin.error((error as Error).message || '加载工单统计失败')
  } finally {
    loading.value = false
  }
}

onMounted(loadStats)
</script>

<style lang="css">
@import '../shared.css';
</style>

<style scoped>
.chart-empty {
  display: flex;
  align-items: center;
  justify-content: center;
  height: 300px;
}
</style>
