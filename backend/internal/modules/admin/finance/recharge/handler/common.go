...
import (
	...
	"hostsent/backend/internal/modules/admin/finance/recharge/service"
...
// writeError 将充值子域业务错误映射为统一错误码。
func writeError(err error) *apperrors.AppError {
	switch {
	case errors.Is(err, service.ErrRechargeNotFound),
		errors.Is(err, accountservice.ErrWalletNotFound):
	...
	case errors.Is(err, accountservice.ErrBizExist):
