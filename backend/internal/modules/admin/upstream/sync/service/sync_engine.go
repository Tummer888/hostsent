...
// syncProducts 拉取上游商品并幂等写入 upstream_products。
func (e *SyncEngine) syncProducts(ctx context.Context, task *syncmodel.SyncTask) error {
	...
	rows := make([]model.UpstreamProduct, 0, len(items))
...
func (e *SyncEngine) convertProduct(providerID uint64, p *pkgmodel.StandardProduct) model.UpstreamProduct {
	specJSON, _ := json.Marshal(p.Specs)
	rawJSON, _ := json.Marshal(p.RawSpecs)
	return model.UpstreamProduct{
		ProviderID: providerID,
