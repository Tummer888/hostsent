 ...
 *
 * 用法：
 *   const { pagination, mobilePage, applyTotal, handlePageChange, goMobilePage,
 *           handleMobilePageSizeChange, resetPage } = useMobilePagination(loadList)
 *   // loadList 里把后端 total 交给 applyTotal(total)
 *
...
export function useMobilePagination(
  ...
  function goMobilePage(target: number) {
    const clamped = Math.min(
      Math.max(target, 1),
      Math.max(1, Math.ceil(mobilePage.total / mobilePage.pageSize)),
    )
